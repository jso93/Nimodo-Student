...
function scene:crearMenu( )
    ...
    scene.incorrecto = display.newImage('src/images/incorrecto.png',10,display.contentCenterX ,display.contentHeight-25 )
    ...
    if scene.filenameImage~=nil then 
        scene.preguntaImagen = display.newImage(scene.filenameImage, scene.baseDirectory,display.contentCenterX,display.contentCenterY+20 )
    ...
    scene.btnPlay = display.newImage('src/images/playButton.png',display.contentCenterX,display.contentCenterY )
    scene.btnPlay.width = 30
    scene.btnPlay.height = 30
    scene.btnPlay.id = 'play'

    scene.btnPause = display.newImage('src/images/pauseButton.png',display.contentCenterX-45,display.contentCenterY+25 )
    scene.btnPause.width = 30
    scene.btnPause.height = 30
    scene.btnPause.id = 'pause'

    scene.btnStop = display.newImage('src/images/stopButton.png',display.contentCenterX+45,display.contentCenterY+25 )
    scene.btnStop.width = 30
    scene.btnStop.height = 30
    scene.btnStop.id = 'stop'
    ...
    if scene.filenameImage~=nil then 
        ...
        scene.btnPlay.x,scene.btnPlay.y = 40,display.contentCenterY-20
        scene.btnPause.x,scene.btnPause.y = 40,display.contentCenterY+20
        scene.btnStop.x,scene.btnStop.y = 40,display.contentCenterY+60
    else 
        print('pregunta sin imagen')
    end
    scene.group:insert(scene.calificar)
    scene.group:insert(scene.calificar.label)
    scene.group:insert(scene.btnPlay)
    scene.group:insert(scene.btnPause)
    scene.group:insert(scene.btnStop)
    ...
    print('cant correcta:'..#scene.listaPreguntaRespuestaCorrecta )
    print('cant incorrecta:'..#scene.listaPreguntaRespuestaIncorrecta )
    print('cant alter:'..#scene.listaPreguntaAlternativa )
    print('apriori:'..scene.conocimientoApriori )
end
...
function scene:create( event )
    ...
    print( 'create preguntaAuditivaScene' )
    scene:crearMenu()
end
...
function scene:show( event )
	...
	if phase == 'will' then
    print( 'show preguntaAuditivaScene' )
...
function scene:hide( event )
	local phase = event.phase
	if phase == 'will' then
    print( 'hide preguntaAuditivaScene' )
	end
end

function scene:destroy( event )
    print( 'destroy preguntaAuditivaScene' )
end
